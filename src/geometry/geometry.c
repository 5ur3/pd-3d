...
Geometry *NewCubeGeometry(void) {
    ...
    SetGeometryFace(&g->f[0], 0, 2, 1);
    SetGeometryFace(&g->f[1], 1, 2, 3);
    SetGeometryFace(&g->f[2], 4, 5, 6);
    SetGeometryFace(&g->f[3], 5, 7, 6);
    SetGeometryFace(&g->f[4], 0, 1, 4);
    SetGeometryFace(&g->f[5], 1, 5, 4);
    SetGeometryFace(&g->f[6], 2, 6, 3);
    SetGeometryFace(&g->f[7], 3, 4, 7);
    SetGeometryFace(&g->f[8], 0, 4, 2);
    SetGeometryFace(&g->f[9], 2, 4, 6);
    SetGeometryFace(&g->f[10], 1, 3, 5);
    SetGeometryFace(&g->f[11], 3, 7, 5);
